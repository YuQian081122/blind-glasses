...
> **目錄約定**（整包專案／本機想維持 `firmware/` 資料夾名稱時必讀）：[docs/REPOSITORY_LAYOUT.md](docs/REPOSITORY_LAYOUT.md)
...
- **本目錄即 PlatformIO 專案根**（內含 `include/`、`src/`、`platformio.ini`）。GitHub 上亦為**根目錄扁平結構**，請勿把韌體再包進 `firmware/` 子目錄後推送。
- 若你希望本機目錄「永遠叫 `firmware/`」且與 `server/` 並列，請見 **[docs/REPOSITORY_LAYOUT.md](docs/REPOSITORY_LAYOUT.md)**（建議單獨 clone 本倉庫到 `…/firmware/` 資料夾）。
- 伺服器（FastAPI）不保證與本倉庫同步發佈；維護韌體請以本 README 與 `include/`、`src/`、`platformio.ini` 為準。

## 快速開始（Windows）

在**本目錄**（與 `platformio.ini` 同層）執行：
...
1. Clone 韌體倉庫後，**工作目錄設為含 `platformio.ini` 的那一層**（即 clone 根目錄；整包與 `server/` 並列時請參考 [docs/REPOSITORY_LAYOUT.md](docs/REPOSITORY_LAYOUT.md)）
