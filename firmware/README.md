...
- **本目錄即 PlatformIO 專案根**（內含 `include/`、`src/`、`platformio.ini`）。若從 GitHub 單獨 clone 韌體倉庫，clone 下來的根目錄就是這一層（不會再多一層 `firmware/`）。
- 若你手邊是「整包儲存」（同層還有 `server/` 等），韌體通常在本機的 `firmware/` 子資料夾內，請在該層執行 `pio run`。
...
1. Clone 韌體倉庫後，**工作目錄設為含 `platformio.ini` 的那一層**（單獨 clone 時即 clone 根目錄；整包專案時為 `firmware/`）
